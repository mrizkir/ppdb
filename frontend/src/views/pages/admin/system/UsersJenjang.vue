                    ...
                    >
                        <template v-slot:top>
                            <v-toolbar flat color="white">
                                <v-toolbar-title>DAFTAR USERS JENJANG STUDI</v-toolbar-title>
                                <v-divider
                                    class="mx-4"
                                    inset
                                    vertical
...
export default {
    ...
    methods: {
        ...
        deleteItem(item) {
            this.$root.$confirm.open('Delete', 'Apakah Anda ingin menghapus username '+item.username+' ?', { color: 'red' }).then((confirm) => {
                ...
                {
                    this.btnLoading = true;
                    this.$ajax.post('/system/usersjenjang/'+item.id,
                        {
                            '_method': 'DELETE',
                        },
                        {
    ...
    computed: {
        formTitle() {
            return this.editedIndex === -1 ? 'TAMBAH USER JENJANG STUDI' : 'EDIT USER JENJANG STUDI'
        },
        ...mapGetters('auth',{
